...
# 얼굴의 물리적 높이를 센티미터 단위로 정의
face_height_centimeters = 23
    ...
    for i in range(detections.shape[2]):
        if faces_count < max_faces:
            ...
            if confidence > 0.9:
                ...
                # 원근 변환 공식을 사용하여 얼굴까지의 거리 계산
                distance = int((face_height_centimeters * focal_length) / (y2 - y))

                # 거리값 표시
                distance_text = f"Face {faces_count} Distance: {distance} cm"
    ...
    if current_time - last_packet_time >= packet_interval:
        ...
        for i in range(4):
            ...
            if i < len(distance_list):
                packet.extend([(distance_list[i] >> 8) & 0xFF, distance_list[i] & 0xFF])
            else:
                packet.extend([0, 0])  # 감지가 안된 데이터는 0으로 채움
